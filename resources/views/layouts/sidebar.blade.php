<aside class="w-64 bg-slate-900 min-h-screen text-slate-300 p-4">
    ...
    <nav class="space-y-2">
        ...
        @if(in_array(Auth::user()->role, ['admin', 'staff', 'manager_it']))
        <div class="pt-4 pb-2 px-4 text-xs font-semibold text-slate-500 uppercase tracking-widest">Operasional</div>

        @if(in_array(Auth::user()->role, ['admin', 'staff']))
        <a href="{{ route('tickets.create') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-800 transition-all">
